...
export default class CTF {
  ...
  static async createCTF(client: Client, name: string, guildSnowflake: string) {
    ...
    const {
      rows: existingCTFs,
    } = await query('SELECT id FROM ctfs WHERE guild_snowflake = $1', [
      guildSnowflake,
    ]);
    if (existingCTFs && existingCTFs.length > 0)
      throw new Error('cannot create a second CTF in this guild');

    const {
      rows: existingNames,
    } = await query('SELECT id FROM ctfs WHERE name = $1', [name]);
    if (existingNames && existingNames.length > 0)
      throw new Error('cannot create ctf with a duplicate name');

    const {
      rows,
    } = await query(
      'INSERT INTO ctfs(name, guild_snowflake) VALUES ($1, $2) RETURNING *',
      [name, guildSnowflake],
    );
    ...
    await (tos as TextChannel)
      .createWebhook('Terms of Service')
      .then(async (webhook) => {
  ...
  async getScoreboard(category?: string) {
    const { rows } = await query(
      `SELECT * FROM teams WHERE ctf_id = ${this.row.id}`,
    );
    const teams = rows.map(
      async (teamRow) => await new Team(teamRow).getMinimalTeam(category),
    );
    // eslint-disable-next-line @typescript-eslint/unbound-method
    const minimalTeams = (await Promise.all(teams))
      .sort(Team.teamCompare.bind(this))
      .slice(0, 20);
    logger(
      minimalTeams
        .map((team) => `${team.name} has ${team.points} with ${team.accuracy}`)
        .toString(),
    );
  }
  ...
  static async fromWebhookCTF(tos_webhook_snowflake: string) {
    const {
      rows,
    } = await query('SELECT * FROM ctfs WHERE tos_webhook_snowflake = $1', [
      tos_webhook_snowflake,
    ]);
    if (rows.length === 0)
      throw new Error('no ctf associated with that webhook');
  ...
  async setInfoCategory(info_category_snowflake: string) {
    const {
      rows,
    } = await query('SELECT id FROM ctfs WHERE info_category_snowflake = $1', [
      info_category_snowflake,
    ]);
    if (rows && rows.length > 0)
      throw new Error('ctf already uses that category');
    await query(
      `UPDATE ctfs SET info_category_snowflake = $1 WHERE id = ${this.row.id}`,
      [info_category_snowflake],
    );
  ...
  async setTOSChannel(tos_channel_snowflake: string) {
    const {
      rows,
    } = await query('SELECT id FROM ctfs WHERE tos_channel_snowflake = $1', [
      tos_channel_snowflake,
    ]);
    if (rows && rows.length > 0)
      throw new Error('ctf already uses that channel');
    await query(
      `UPDATE ctfs SET tos_channel_snowflake = $1 WHERE id = ${this.row.id}`,
      [tos_channel_snowflake],
    );
  ...
  async setTOSWebhook(tos_webhook_snowflake: string) {
    const {
      rows,
    } = await query('SELECT id FROM ctfs WHERE tos_webhook_snowflake = $1', [
      tos_webhook_snowflake,
    ]);
    if (rows && rows.length > 0)
      throw new Error('ctf already uses that webhook');
    await query(
      `UPDATE ctfs SET tos_webhook_snowflake = $1 WHERE id = ${this.row.id}`,
      [tos_webhook_snowflake],
    );
  ...
  static async fromCTFGuildSnowflakeCTF(guild_snowflake: string) {
    const {
      rows,
    } = await query('SELECT * FROM ctfs WHERE guild_snowflake = $1', [
      guild_snowflake,
    ]);
  ...
  async setDescription(description: string) {
    await query(`UPDATE ctfs SET description = $1 WHERE id = ${this.row.id}`, [
      description,
    ]);
    this.row.description = description;
    if (description !== '')
      logger(`Set ${this.row.name}'s description to "${description}"`);
  }

  async setStartDate(startDate: Date) {
    await query(`UPDATE ctfs SET start_date = $1 WHERE id = ${this.row.id}`, [
      startDate,
    ]);
  ...
  async setEndDate(endDate: Date) {
    await query(`UPDATE ctfs SET end_date = $1 WHERE id = ${this.row.id}`, [
      endDate,
    ]);
  ...
  async setAdminRoleSnowflake(adminRoleSnowflake: string) {
    await query(
      `UPDATE ctfs SET admin_role_snowflake = $1 WHERE id = ${this.row.id}`,
      [adminRoleSnowflake],
    );
  ...
  async setAnnouncementsChannelSnowflake(
    announcementsChannelSnowflake: string,
  ) {
    await query(
      `UPDATE ctfs SET announcements_channel_snowflake = $1 WHERE id = ${this.row.id}`,
  ...
  async createCategory(client: Client, name: string) {
    ...
    if (existingRows && existingRows.length > 0)
      throw new Error(
        'cannot create a category with a duplicate name in this CTF',
      );
  ...
  async fromNameCategory(name: string) {
    const {
      ...
      `SELECT * FROM categories WHERE name = $1 and ctf_id = ${this.row.id}`,
      [name],
    );
    if (rows.length === 0)
      throw new Error('no category with that name in this ctf');
    return new Category(rows[0] as CategoryRow, this);
  }

  async fromChannelCategorySnowflakeCategory(
    ...
    if (rows.length === 0)
      throw new Error('no category with that snowflake in this ctf');
    return new Category(rows[0] as CategoryRow, this);
  }

  async getAllCategories() {
    const { rows } = await query(
      `SELECT * FROM categories WHERE ctf_id = ${this.row.id}`,
    );
  ...
  async fromChannelSnowflakeChallenge(channel_snowflake: string) {
    const {
      rows,
    } = await query(`SELECT * FROM challenges WHERE channel_snowflake = $1`, [
      channel_snowflake,
    ]);
    if (rows.length === 0)
      throw new Error('no challenge with that channel in this ctf');
    return new Challenge(rows[0] as ChallengeRow, this);
  }
  ...
  async createTeamServer(guild: Guild, name: string, team_limit: number) {
    ...
    if (existingRows && existingRows.length > 0)
      throw new Error(
        'cannot create a team server with a duplicate name in this ctf',
      );

    const {
      rows: existingRows2,
    } = await query('SELECT id FROM team_servers WHERE guild_snowflake = $1', [
      guild.id,
    ]);
    if (existingRows2 && existingRows2.length > 0)
      throw new Error('guilds are limited to a single teamserver');
    ...
    await teamServer.setTeamCategorySnowflake(
      (await teamServer.makeCategory(guild.client, 'Teams')).id,
    );
  ...
  async fromNameTeamServer(name: string) {
    const {
      ...
      `SELECT * FROM team_servers WHERE name = $1 and ctf_id = ${this.row.id}`,
      [name],
    );
    if (rows.length === 0)
      throw new Error('no team server with that name in this ctf');
  ...
  async fromGuildSnowflakeTeamServer(guild_snowflake: string) {
    const {
      ...
      `SELECT * FROM team_servers WHERE guild_snowflake = $1 and ctf_id = ${this.row.id}`,
      [guild_snowflake],
    );
    if (rows.length === 0)
      throw new Error('no team server with that snowflake in this ctf');
    return new TeamServer(rows[0] as TeamServerRow);
  }
  ...
  async printAllTeams() {
    ...
    return teams.length == 0
      ...
      : `CTF **${this.row.name}**'s teams are: **${teams
          .map((team) => team.row.name)
          .join('**, **')}**`;
  }
  ...
  static async fromTeamServerGuildSnowflakeTeamServer(guild_snowflake: string) {
    const {
      rows,
    } = await query('SELECT * FROM team_servers WHERE guild_snowflake = $1', [
      guild_snowflake,
    ]);
    if (rows.length === 0)
      throw new Error('no team server with that snowflake');
    return new TeamServer(rows[0] as TeamServerRow);
  }

  static async fromIdTeamServer(team_server_id: number) {
    const { rows } = await query(
      `SELECT * FROM team_servers WHERE id = ${team_server_id}`,
    );
  ...
  async getAllTeamServers() {
    const { rows } = await query(
      `SELECT * FROM team_servers WHERE ctf_id = ${this.row.id}`,
    );
  ...
  async createUser(member: GuildMember) {
    ...
    const {
      ...
      `INSERT INTO users(ctf_id, user_snowflake) VALUES (${this.row.id}, $1) RETURNING *`,
      [member.user.id],
    );
    ...
    while (!nameAvailable) {
      try {
        team = await teamServer.makeTeam(
          member.client,
          this,
          `${teamName}${iteration > 1 ? ` ${iteration}` : ''}`,
        );
        nameAvailable = true;
      } catch (e) {
    ...
    if (this.row.guild_snowflake !== teamServer.row.guild_snowflake) {
      user = member.client.guilds
        .resolve(teamServer.row.guild_snowflake)
  ...
  async fromUserSnowflakeUser(user_snowflake: string) {
    const {
      ...
      `SELECT * FROM users WHERE user_snowflake = $1 and ctf_id = ${this.row.id}`,
      [user_snowflake],
    );
  ...
  static async fromGuildSnowflakeCTF(guild_id: string) {
    ...
    try {
      ...
      ctf = await CTF.fromIdCTF(
        (await CTF.fromTeamServerGuildSnowflakeTeamServer(guild_id)).row.ctf_id,
      );
    } catch {
    ...
    if (roleToDelete) {
      ...
      logger(
        `**${
          client.guilds
            .resolve(this.row.guild_snowflake)
            .roles.resolve(role_snowflake).name
  ...
  async fromUserTeam(user_snowflake: string) {
    const teamServers = await this.getAllTeamServers();
    let team: Team;
    const teamID = (await this.fromUserSnowflakeUser(user_snowflake)).row
      .team_id;
