...
export default class Challenge {
  ...
  async setName(client: Client, name: string) {
    await query(`UPDATE challenges SET name = $1 WHERE id = ${this.row.id}`, [
      name,
    ]);
  ...
  async setAuthor(author: string) {
    await query(`UPDATE challenges SET author = $1 WHERE id = ${this.row.id}`, [
      author,
    ]);
  ...
  async setPrompt(prompt: string) {
    await query(`UPDATE challenges SET prompt = $1 WHERE id = ${this.row.id}`, [
      prompt,
    ]);
  ...
  async setDifficulty(difficulty: string) {
    await query(
      `UPDATE challenges SET difficulty = $1 WHERE id = ${this.row.id}`,
      [difficulty],
    );
  ...
  async setInitialPoints(initial_points: number) {
    await query(
      `UPDATE challenges SET initial_points = $1 WHERE id = ${this.row.id}`,
      [initial_points],
    );
  ...
  async setPointDecay(point_decay: number) {
    await query(
      `UPDATE challenges SET point_decay = $1 WHERE id = ${this.row.id}`,
      [point_decay],
    );
  ...
  async setMinPoints(min_points: number) {
    await query(
      `UPDATE challenges SET min_points = $1 WHERE id = ${this.row.id}`,
      [min_points],
    );
  ...
  async setFlag(flag: string) {
    await query(`UPDATE challenges SET flag = $1 WHERE id = ${this.row.id}`, [
      flag,
    ]);
    this.row.flag = flag;
  }
  ...
  async setFirstBlood(first_blood_id: number) {
    await query(
      `UPDATE challenges SET first_blood_id = $1 WHERE id = ${this.row.id}`,
      [first_blood_id],
    );
  ...
  async setPublishTime(publish_time: Date) {
    await query(
      `UPDATE challenges SET publish_time = $1 WHERE id = ${this.row.id}`,
      [publish_time],
    );
  ...
  async createAttachment(name: string, url: string) {
    ...
    const {
      ...
      `SELECT id FROM attachments WHERE name = $1 and challenge_id = ${this.row.id}`,
      [name],
    );
    if (existingRows && existingRows.length > 0)
      throw new Error('cannot create an attachment with a duplicate name');

    const {
      rows,
    } = await query(
      `INSERT INTO attachments(challenge_id, name, url) VALUES (${this.row.id}, $1, $2) RETURNING *`,
      [name, url],
    );
  ...
  async fromNameAttachment(name: string) {
    const {
      ...
      `SELECT * FROM attachments WHERE name = $1 and challenge_id = ${this.row.id}`,
      [name],
    );
    return new Attachment(rows[0] as AttachmentRow);
  }
  ...
  async getAllAttachments() {
    const { rows } = await query(
      `SELECT * FROM attachments WHERE challenge_id = ${this.row.id}`,
    );
  ...
  async getAllAttempts() {
    const { rows } = await query(
      `SELECT * FROM attempts WHERE challenge_id = ${this.row.id}`,
    );
  ...
  async getCategory() {
    const { rows } = await query(
      `SELECT * FROM categories WHERE id = ${this.row.category_id}`,
    );
  ...
  getChannelChallenge(client: Client) {
    const channel = this.ctf
      .getGuild(client)
      .channels.resolve(this.row.channel_snowflake);
    if (!channel)
      throw new Error('No channel corresponding with this challenge id found.');
    return channel;
  }
}
