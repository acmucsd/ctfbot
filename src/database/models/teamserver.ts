...
export default class TeamServer {
  ...
  async deleteTeamServer(client: Client) {
    ...
    teams.forEach((team) => {
      ...
      void guild.roles
        .resolve(team.row.team_role_snowflake_team_server)
        .delete();
    });
  ...
  async deleteChannel(client: Client, channel_snowflake: string) {
    ...
    if (channel) {
      await channel.delete();
      logger(
        `Channel with id ${channel_snowflake} found: deleted that channel`,
      );
      return;
    }
  ...
  async hasSpace(print?: boolean) {
    ...
    if (print)
      logger(
        `Team server **${this.row.name}** has${hasSpace ? ' ' : ' no '}space`,
      );
    return hasSpace;
  }
